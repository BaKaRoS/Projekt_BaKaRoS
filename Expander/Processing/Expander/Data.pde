public class Data{
  ...
  public Data(int sensorNumber_){
    ...
    this.lastRollValues = new float[10];
    this.lastRollIndex = 0;
    this.lastPressureValues = new int[10];
  }
  ...
  public void addData(float battery_, int weight_, int accX_, int accY_, int accZ_, int gyrX_, int gyrY_, int gyrZ_, float temp_){
    ...
    //track roll angle
    this.lastRollValues[lastRollIndex] = this.getRollDegree();
    
    this.lastRollIndex++;
    if(this.lastRollIndex >= 10){
      this.lastRollIndex = 0;
    }
    
    //track pressure values
    for(int i = 0 ; i < 9; i++){
      this.lastPressureValues[i] = this.lastPressureValues[i+1];
    }
    this.lastPressureValues[9] = this.getWeight();
  ...
  public float[] getLastRollValues(){
    return this.lastRollValues;
  }
  
  public int[] getLastPressureValues(){
    return this.lastPressureValues;
  }
  ...
  //shoulder rotation counting
  private float lastRollValues[];
  private int lastRollIndex;
  
  //shoulder rotation counting
  private int lastPressureValues[];
